...
const SignUpPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (role === 'ADMIN') {
      setError('Tài khoản ADMIN chỉ được cấp bởi Quản trị viên hệ thống. Vui lòng chọn Khách tham dự hoặc Nhà tổ chức.');
      return;
    }
  ...
  return (
    <div className="flex h-screen w-full overflow-hidden bg-[#F8FAFC] font-sans layout-fixed">
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-6 sm:p-8 lg:p-10 xl:p-12 bg-white overflow-y-auto font-sans h-full">
        <div className="w-full max-w-[440px] my-auto py-4 animate-in fade-in slide-in-from-right-4 duration-700">
          <div className="mb-5">
            <h2 className="text-3xl font-bold text-gray-900 mb-1 tracking-tight">Tạo tài khoản của bạn</h2>
            <p className="text-gray-500 font-medium text-sm">Nhập thông tin chi tiết để bắt đầu hành trình.</p>
          </div>

          {error && (
            <div className="mb-4 p-3.5 bg-rose-50 border border-rose-100 rounded-xl text-rose-600 text-xs font-medium animate-in fade-in slide-in-from-top-2">
              {error}
            </div>
          )}
          ...
          <form className="space-y-3.5" onSubmit={handleSubmit}>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div className="group">
                <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1.5 ml-1" htmlFor="name">Họ và Tên</label>
                <div className="relative">
                  ...
                  <input 
                    ...
                    className="block w-full pl-10 pr-3.5 py-2.5 bg-gray-50 border-2 border-transparent rounded-xl focus:bg-white focus:border-indigo-100 focus:ring-4 focus:ring-indigo-500/5 transition-all outline-none text-gray-900 font-medium text-sm"
                    required
                  />
                </div>
              </div>
              ...
              {role === 'ORGANIZER' && (
                <div className="group animate-in fade-in slide-in-from-right-2">
                  <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1.5 ml-1" htmlFor="company">Tên Công ty</label>
                  <div className="relative">
                    <div className="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none">
            ...
            <div>
              <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1.5 ml-1">Đăng ký với tư cách</label>
              <div className="flex gap-3">
                <button
                  type="button"
                  onClick={() => setRole('ATTENDEE')}
                  className={`flex-1 py-2.5 px-3 rounded-xl font-bold text-xs transition-all duration-300 border-2 cursor-pointer ${
                    role === 'ATTENDEE'
              ...
              <p className="text-[11px] text-gray-400 mt-1.5 ml-1 font-medium italic">
                * Đăng ký tự do dành cho Khách tham dự & Nhà tổ chức. Quản trị viên (Admin) được khởi tạo từ trang quản trị.
              </p>
            </div>

            <div className="group">
              <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1.5 ml-1" htmlFor="email">Email Công việc</label>
              <div className="relative">
                <div className="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none">
            ...
            <div className="group">
              <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1.5 ml-1" htmlFor="password">Mật khẩu</label>
              <div className="relative">
                <div className="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none">
                ...
                <input 
                  ...
                  placeholder="••••••••"
                  className="block w-full pl-10 pr-10 py-2.5 bg-gray-50 border-2 border-transparent rounded-xl focus:bg-white focus:border-indigo-100 focus:ring-4 focus:ring-indigo-500/5 transition-all outline-none text-gray-900 font-medium text-sm"
                  required
                />
                <button 
                  type="button"
                  onClick={() => setShowPassword(!showPassword)}
                  className="absolute inset-y-0 right-0 pr-3.5 flex items-center text-gray-400 hover:text-indigo-600 transition-colors cursor-pointer"
                >
            ...
            <div>
              <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1.5 ml-1">Loại Sự kiện Ưu tiên</label>
            ...
            <div className="flex items-center group cursor-pointer" onClick={() => setAgreed(!agreed)}>
              <div className={`w-4 h-4 rounded border-2 flex items-center justify-center transition-all ${agreed ? 'bg-indigo-600 border-indigo-600' : 'bg-gray-50 border-gray-200'}`}>
                {agreed && <Check className="w-3 h-3 text-white" strokeWidth={3} />}
              </div>
              <span className="ml-2.5 text-xs font-medium text-gray-600 select-none">
                Tôi đồng ý với <a href="#" className="font-bold text-indigo-600 hover:underline">Điều khoản</a> và <a href="#" className="font-bold text-indigo-600 hover:underline">Bảo mật</a>.
              </span>
            </div>
            ...
            <button 
              ...
              className="w-full bg-indigo-600 text-white py-3 px-5 rounded-xl font-bold text-sm hover:bg-indigo-700 hover:shadow-xl hover:shadow-indigo-500/20 active:scale-[0.98] transition-all duration-300 flex items-center justify-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed cursor-pointer"
            >
              {isSubmitting ? (
                <>
          ...
          <div className="relative my-4 text-center">
            ...
            <span className="relative bg-white px-4 text-[10px] font-black text-gray-400 uppercase tracking-widest">hoặc tiếp tục với</span>
          </div>

          <div className="flex gap-3 mb-4">
            <button className="flex-1 flex items-center justify-center gap-2 py-2.5 px-3 bg-white border border-gray-100 rounded-xl hover:bg-gray-50 transition-all font-bold text-xs text-gray-700 shadow-sm cursor-pointer">
          ...
          <p className="text-center text-gray-500 text-xs font-medium">
            Đã có tài khoản? {' '}
            <button onClick={() => navigate('/login')} className="font-bold text-indigo-600 hover:text-indigo-800 transition-colors cursor-pointer">Đăng nhập</button>
          </p>
        </div>
      </div>
