...
| Admin settings – security/API/branding | 2FA, session policy, SMTP, branding, backup | `/api/v1/admin/settings` legacy or local simulation | None approved by frozen contracts | None verified for these keys | N/A | No response mapping | Blocker toast; no fake persistence/download | Manual blocker verification | Blocked; no shared settings endpoint may be invented |
